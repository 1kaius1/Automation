...
- `.claude/rules/git-workflow.md` — added a rule requiring the user's explicit instruction before merging any pull request; opening or reviewing a PR does not imply authorization to merge it, and merging stays a manual step done via the GitHub web interface.
- `CLAUDE.md` — added an Idempotency section (verify by running a playbook twice and confirming `changed=0` on the second pass, not just that the first run succeeded) and a rule against printing decrypted Vault content anywhere a log or shell history would retain it. Both were already practiced ad hoc (e.g. the idempotency check in `Proxmox-Steam-Linux`'s PR) but not previously written down as a rule.
